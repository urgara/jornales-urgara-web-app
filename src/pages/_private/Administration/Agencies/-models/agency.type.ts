...
const AgencySchema = z.object({
	id: z.string().uuid(),
	name: z.string().min(1, 'El nombre es requerido').max(150, 'El nombre no debe exceder 150 caracteres'),
...
const ListAgenciesResponseSchema = ResponseGenericIncludeDataAndPaginationSchema(ListAgenciesSchema);
...
const UpdateAgencyRequestSchema = z.object({
	name: z.string().min(1, 'El nombre es requerido').max(150, 'El nombre no debe exceder 150 caracteres').optional(),
});
